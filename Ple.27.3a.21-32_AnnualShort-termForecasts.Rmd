...
The model selected for creating forecasts and advice, this year, is called [ple.27.21.32_WGBFAS_2026_SA](https://stockassessment.org/setStock.php?stock=ple.27.21.32_WGBFAS_2026_SA) and is available from [stockassessment.org](https://stockassessment.org).
...
# Discard rates applied as fractions of number ----
survfrac <- read.ices("Assessment Input/StockAnnex/df_frac.dat")
survfracTotal <- survfrac
...
### Calculate discard survival rate by age for 1 -> 7
survfrac[, E] <- rowSums(survfrac[, GE] * w)
survfrac <- survfrac[, low:E]

## Weighted means of surviving fraction by number, collapsing age to give an individual number per year
cutage <- 1
low <- 1
GE1 <- which(as.numeric(colnames(catch.no)) >= cutage)
E1 <- which(as.numeric(colnames(catch.no)) == cutage)
w1 <- catch.no[, GE1]/rowSums(catch.no[, GE1])
wex1 <- rbind(w1, w1[nrow(w1), ])
### Calculate discard survival rate by numbers, collapsed ages for all years
survfracTotal[, E1] <- rowSums(survfracTotal[, GE1] * w1)
survfracTotal <- survfracTotal[, low:E1]

# Discard rates applied as fractions of mass, not fraction of number ----
...
allCatchMass <- rowSums(totCatchMass)
...
recyears <- (DataYear-4):DataYear
catSurvFracAve <- mean(catFracTot[(length(catFracTot)-(bioAveYears-1)):length(catFracTot)])
...
FC[[length(FC)+1]] <- forecast(fit_CURRENT,
                               fscale=c(1,1,NA,NA),
                               catchval=c(NA,NA,
                                          unname(LastIcesAdvice-(LastIcesAdvice*catFracTot[length(catFracTot)]))*1.2,
                                          unname(LastIcesAdvice-(LastIcesAdvice*catFracTot[length(catFracTot)]))*1.2), 
                               label=paste0("Catch(", DataYear+2, ")<=20% Advice(", DataYear+1, ")[Discard Survival(", DataYear, ")]"),
                               ave.years=(DataYear-bay):(DataYear+1),
                               rec.years=recyears,
                               year.base=DataYear,
                               splitLD=TRUE,
...
below<-function(x){xx<-mean(x[[4]]$ssb<Blim);yr<-x[[4]]$year; round(xx*100,1)}
Pbelow<-lapply(FC, below)
names(Pbelow)<-lapply(FC, function(x){attr(x,"label")})
Pbelow<-as.matrix(unlist(Pbelow))
colnames(Pbelow)<-paste0("P(SSB",FC[[1]][[4]]$year, " < ",Blim,") in %")
...
```{r forecastSurvivors, eval=TRUE}
# Add survivors back to model estimations ----
## Initialise new ASUM
asum_Survivors <- asum_CURRENT[asum_CURRENT$Year != (DataYear+1), ]
asum_Survivors$CatchSurv <- unname(allCatchMass)
asum_Survivors$CatchSurvHigh <- asum_Survivors$Catchhigh+(asum_Survivors$CatchSurv - asum_Survivors$CatchEst)
asum_Survivors$CatchSurvLow <- asum_Survivors$Catchlow+(asum_Survivors$CatchSurv - asum_Survivors$CatchEst)

## Re-extract Full Catch Observations
catch.no<-read.ices('Assessment Input/StockAnnex/cn.dat')/1000
...
### Add survivors back to forecast catch
ForcTable$`survCatch:median` <- ForcTable$`catch:median`/(1-catSurvFracAve)
ForcTable$`survCatch:low`<- ForcTable$`catch:low`+(ForcTable$`survCatch:median`-ForcTable$`catch:median`)
ForcTable$`survCatch:high` <- ForcTable$`catch:high`+(ForcTable$`survCatch:median`-ForcTable$`catch:median`)
```
...
intyear <- data.frame(Variable = c("F~ages 3-6~",
                                   ...
                                   paste0("Discard Survival Rates (", (DataYear+1), ")"),
                                   paste0("Discard Survival Rate (", (DataYear+1), ")"),
                                   paste0("Projected Surviving Discards (", (DataYear+1), ")"),
                                   paste0("Projected Dead Discards (", (DataYear+1), ")")),
                      Value = c(asum_CURRENT[asum_CURRENT$Year == (DataYear),"Fbar"],
                                ...
                                round(ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "survCatch:median"], 0),
                                ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "Land:median"],
                                round(ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "survCatch:median"] -                                 ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "Land:median"], 0),
                                paste(round(unname(colMeans(survfrac[(nrow(survfrac)-(bioAveYears-1)):nrow(survfrac),])), 2), collapse=", "),
                                paste(icesRound(unname(mean(survfracTotal[(length(survfracTotal)-(bioAveYears-1)):length(survfracTotal)])))),
                                round((ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "survCatch:median"] -                                 ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "Land:median"]) - ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "Discard:median"], 0),
                      ...
                      Notes = c(paste0("F~ages 3-6~ in ", DataYear, "."),
                                ...
                                paste0("Median resampled from the previous five years of estimated recruitment (excluding latest year with only Q1 data ", (DataYear+1), "); thousands."),
                                paste0("Short-term forecast using F(", (DataYear+1), "), plus surviving discards; tonnes."),
                                paste0("Short-term forecast; assuming average landings pattern by age in ", (DataYear-(bioAveYears-1)), "-", DataYear, "; tonnes."),
                                paste0("Short-term forecast; assuming average discard pattern and survival, by age, in ", (DataYear-(bioAveYears-1)), "-", DataYear, ", plus surviving discards; tonnes."),
                                paste0("Survival rates in discards, by age (1-7), in ", (DataYear-(bioAveYears-1)), "-", DataYear, "."),
                                paste0("Survival rate in discards, weighted mean by number across ages, and average over ", (DataYear-(bioAveYears-1)), "-", DataYear, "."),
                                paste0("Short-term forecast; assuming an average discard pattern and survival, by age, in ", (DataYear-(bioAveYears-1)), "-", DataYear, ", plus surviving discards; tonnes."),
                                paste0("Short-term forecast; assuming an average discard pattern and survival, by age, in ", (DataYear-(bioAveYears-1)), "-", DataYear, ", plus surviving discards; tonnes.")
                      )
)
...
```{r fig.cap= "Catches for ple.27.21-32 and 95% confidence intervals (tonnes). Blue is the assessment model estimation with surviving discards added back, while green points are the forecasts with surviving discards added back to the total catches, starting from the last full year of data. Orange points are the observations with surviving discards."}
psurvcatch <- ggplot() +
  geom_line(data = asum_Survivors,
            mapping = aes(x=Year,
                          y=CatchSurv,
                          series="Estimates with survivors"),
            colour = ebpal[8]) +
  ...
  geom_point(data = asum_Survivors,
             mapping = aes(x=Year,
                           y=CatchObsSurv,
                           series="Observed Catches with Survivors"),
             shape = 3,
             colour = ebpal[10],
             fill = ebpal[10]) +
  theme_clean()

ggplotly(psurvcatch, tooltip = c("series", "x", "y"))
```
...
kable(cotable,
      caption = "DEAD Catch Options Table; Format based on old ple.21-23 stock,formatted for advice sheet (ICES rounding rules applied), without having surviving discard fraction of the catch added back.",
...
for(i in unique(ForcTable$scenario)){
  a <- ForcTable[ForcTable$scenario == i & ForcTable$Year == (DataYear+2), "scenario"]
  b <- ForcTable[ForcTable$scenario == i & ForcTable$Year == (DataYear+2), "survCatch:median"]
  c <- ForcTable[ForcTable$scenario == i & ForcTable$Year == (DataYear+2), "Land:median"]
  d <- (ForcTable[ForcTable$scenario == i & ForcTable$Year == (DataYear+2), "survCatch:median"]-ForcTable[ForcTable$scenario == i & ForcTable$Year == (DataYear+2), "Land:median"])
  e <- ForcTable[ForcTable$scenario == i & ForcTable$Year == (DataYear+2), "fbar:median"]
  f <- ForcTable[ForcTable$scenario == i & ForcTable$Year == (DataYear+3), "ssb:median"]
  g <- (ForcTable[ForcTable$scenario == i & ForcTable$Year == (DataYear+3), "ssb:median"] - ForcTable[ForcTable$scenario == i & ForcTable$Year == (DataYear+2), "ssb:median"])/ForcTable[ForcTable$scenario == i & ForcTable$Year == (DataYear+2), "ssb:median"]
  h <- (ForcTable[ForcTable$scenario == i & ForcTable$Year == (DataYear+2), "survCatch:median"] - LastIcesAdvice)/LastIcesAdvice
  
  cotable <- rbind(cotable, data.frame(a, b, c, d, e, f, g, h))
}

cotable$pbelowBlim <- unname(Pbelow[, 1])

colnames(cotable) <- c("Basis",
                       paste0("TotalCatch_",(DataYear+2)),
                       paste0("Projected Landings_", (DataYear+2)),
                       paste0("Projected Total Discards_", (DataYear+2)),
                       paste0("Total F_", (DataYear+2)),
                       paste0("SSB_", (DataYear+3)),
                       paste0("%SSB_change_", (DataYear+3), "_cf_", (DataYear+2)),
                       paste0("%ChangeAdvice_", (DataYear+2), "_cf_", (DataYear+1)),
                       paste0("P SSB", (DataYear+3), " < Blim"))


cotable[,2] <- round(cotable[,2])
cotable[,3] <- round(cotable[,3])
cotable[,4] <- round(cotable[,4])
cotable[,5] <- icesRound(cotable[,5])
cotable[,6] <- round(cotable[,6])
cotable[,7] <- icesRound(cotable[,7]*100, percent = TRUE, sign = FALSE)
cotable[,8] <- icesRound(cotable[,8]*100, percent = TRUE, sign = FALSE)
cotable[,9] <- round(cotable[,9], 0)

kable(cotable,
      caption = "Catch Options Table (with Surviving Discards) formatted for advice sheet (ICES rounding rules applied)",
      col.names = gsub(x = colnames(cotable), pattern = "_", replacement = " "))
