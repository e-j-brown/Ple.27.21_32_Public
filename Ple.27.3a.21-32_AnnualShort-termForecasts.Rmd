...
survCatchFrac <- read.ices("Assessment Input/StockAnnex/cn_frac.dat")
...
## Boil down to one number per year
cutage<-1
low<-1
...
## Calculate CATCH survival rate for all ages, by number, in recent years (specified by bioAveYears)
survCatchFrac[,E1]<-rowSums(survCatchFrac[,GE1]*w1)
survCatchFrac <- mean(survCatchFrac[(nrow(survCatchFrac)-bioAveYears):nrow(survCatchFrac),low:E1])
...
# Add survivors to forecast catches (discard rates applied as fractions of mass, not fraction of number ----
# Surviving discard fractions from discards and total catch - from DataCoordination Lowestoft file
...
ForcTable$`survCatch:median` <- ForcTable$`catch:median`/(1-catFracTot)
...
intyear <- data.frame(Variable = c("F~ages 3-6~",
                      ...
                      Value = c(asum_CURRENT[asum_CURRENT$Year == (DataYear),"Fbar"],
                                ...
                                ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "survCatch:median"],
                                ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "Land:median"],
                                ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "survCatch:median"] -                                 ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "Land:median"],
                                paste(round(unname(colMeans(survfrac[(nrow(survfrac)-bioAveYears):nrow(survfrac),])), 2), collapse=", "),
                                (ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "survCatch:median"] -                                 ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "Land:median"]) - ForcTable[ForcTable$Year == (DataYear+1) & ForcTable$scenario == "Fmsy", "Discard:median"],
                      ...
                      Notes = c(paste0("F~ages 3-6~ in ", DataYear, "."),
                                ...
                                paste0("Short-term forecast; assuming average landings pattern by age in ", (DataYear-bioAveYears), "-", DataYear, "; tonnes."),
                                paste0("Short-term forecast; assuming average discard pattern and survival, by age, in ", (DataYear-bioAveYears), "-", DataYear, ", plus surviving discards; tonnes."),
                                paste0("Survival rates in discards, by age (1-7), in ", (DataYear-bioAveYears), "-", DataYear, "."),
                                paste0("Short-term forecast; assuming an average discard pattern and survival, by age, in ", (DataYear-bioAveYears), "-", DataYear, ", plus surviving discards; tonnes."),
                                paste0("Short-term forecast; assuming an average discard pattern and survival, by age, in ", (DataYear-bioAveYears), "-", DataYear, ", plus surviving discards; tonnes.")
                      )
)
...
# Splitting Advice for Management Areas
This stock covers ICES subdivisions 21 to 32.  Subdivision 21 (Kattegat), falls under the North Sea Management area and, therefore, has its Total Allowable Catch determined independently of the rest of the stock.  In the past, when this stock was assessed as two components, independent assessments provided estimates of catches, landings and projected advice which were used to determine a splitting ration of said advice, based on the 
