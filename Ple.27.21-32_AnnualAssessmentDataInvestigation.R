...
samptable <- samptable[rowSums(samptable[, c("Catch(tonnes)", "LengthSamples", "LengthsMeasured", "AgeSamples", "AgesRead")]) > 0 & !is.na(samptable$`Catch(tonnes)`), ]
#=====
...
if((file.exists("DataProcessed/Surveys/Ple.27.21-32_ca-hh.RDS") & file.exists("DataProcessed/Surveys/Ple.27.21-32_hl-hh.RDS"))){
  ...
  hl_hh_fin_bits_ibts <- readRDS(file = "DataProcessed/Surveys/Ple.27.21-32_hl-hh.RDS")
}else{
  #===
  # Read in Data ----
  #===
...
#===
k <- append(k, c("ca_hh_fin_bits_ibts", "hl_hh_fin_bits_ibts", "catchNum", "catchcohorts"))
...
ggplot() +
  geom_point(data = tempdf[!is.na(tempdf$AgeF), ],
             mapping = aes(x = AgeF,
                           y = LngtClass)) +
  geom_line(data = tdf[!is.na(tdf$AgeF), ],
            mapping = aes(x = AgeN,
                          y = est_sv),
            colour = ebpal[1],
            size = 2) +
