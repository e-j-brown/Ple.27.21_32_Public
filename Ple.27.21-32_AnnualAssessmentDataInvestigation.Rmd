...
Currently, these indices are generated by Casper Berg from DTU Aqua, using a method he published in 2014. An adaptation of Casper's method is in progress.
...
survTun <- read.csv(file = "DataIn/ple.27.21-23_SurveyTuning_IBTS-BITS_1992-DataYear_2019Q34Excluded.csv", header = T)
k <- append(k, "survTun")
```

```{r eval=FALSE, fig.cap= "NS-IBTS & BITS derived tuning indices for ple.27.21-23 Q1 and Q3/4."}
#===
# Data prep
#====
...
After all of this, these life history parameters do not look acceptable for this species.  Therefore we take values from [fishbase.se](https://fishbase.se/popdyn/PopGrowthList.php?ID=1342&GenusName=Pleuronectes&SpeciesName=platessa&fc=440), such that: $L_{\infty} =$ 52cm and $K =$ 0.168 ($t0 =$ -0.72, in this case).
```{r aselectParams}
...
```{r fig.cap="Time and age varying natural mortality based on Gislason. Time faceted by age."}
...
```{r fig.cap="Time and age varying natural mortality based on Gislason. Ages faceted by time."}
...
save_as_lowestoft(df = nm_SAM[nm_SAM$year %in% c(2002:(DataYear+1)), colnames(nm_SAM)[-11]],
                  file_path = "Assessment Input/AlternateModel_5ysw/nm.dat",
...
```{r fig.cap="Age varying natural mortality based on Gislason (mean values from above time series +/- 95% confidence intervals."}
