...
write.csv(IC_agg_temp, file = paste0("../", DataYear+1, "/Report/Catches_CategoryFleetQuarter.csv"))
#=====
...
### Investigating Weight, Length & Age Relationships in Catches
Here we visualise a range of different relationships, overtime, to see how the inter-realationships between Age, Length, Weight, and the integrated value of Fulton's K condition factor have developed over time. 

```{r lengthWeightAgeData}
#===
# Data Import and shaping
#===
## Read in catch data (before survivor splitting)
cn <- read.ices("Assessment Input/StockAnnex/cn.dat") # Total catch numbers
cwr <- read.ices("Assessment Input/StockAnnex/cw.dat") # Total catch numbers
lf <- read.ices("Assessment Input/StockAnnex/lf.dat") # Landings catch numbers
df <- cn-lf                                           # Discard catch numbers
lwr <- read.ices("Assessment Input/StockAnnex/lw.dat") # Mean individual weight in the landings
dwr <- read.ices("Assessment Input/StockAnnex/dw.dat") # Mean individual weigh in the discards
dmr <- df*dwr                                          # Discard biomass

lw <- make_plus_group(N = lf, M = lwr, cutage = 7)
cw <- make_plus_group(N = cn, M = cwr, cutage = 7)
dw <- make_plus_group(N = df, M = dwr, cutage = 7)
dm <- make_plus_group(N = df, M = dmr, cutage = 7)
dfp <- cbind(df[, 1:6], "7" = rowSums(df[, 7:10]))
cnp <- cbind(cn[, 1:6], "7" = rowSums(cn[, 7:10]))
...
lwdf <- as.data.frame(lw$M)
lwdf$Year <- rownames(lwdf)
rownames(lwdf) <- NULL
lwdf <- melt(lwdf,
             id.vars = "Year",
             variable.name = "Age",
             value.name = "LandingsWeights")

cwdf <- as.data.frame(cw$M)
cwdf$Year <- rownames(cwdf)
rownames(cwdf) <- NULL
cwdf <- melt(cwdf,
             id.vars = "Year",
             variable.name = "Age",
             value.name = "CatchesWeights")

dwdf <- as.data.frame(dw$M)
dwdf$Year <- rownames(dwdf)
rownames(dwdf) <- NULL
dwdf <- melt(dwdf,
             id.vars = "Year",
             variable.name = "Age",
             value.name = "DiscardsWeights")

w_thrdf <- as.data.frame(w_thr)
w_thrdf$Year <- rownames(w_thrdf)
rownames(w_thrdf) <- NULL
w_thrdf <- melt(w_thrdf,
             id.vars = "Year",
             variable.name = "Age",
             value.name = "ThresholdWeights")
w_thrdf <- w_thrdf[!w_thrdf$Year %in% c(DataYear+1), ]

dfpdf <- as.data.frame(dfp)
dfpdf$Year <- rownames(dfpdf)
rownames(dfpdf) <- NULL
dfpdf <- melt(dfpdf,
             id.vars = "Year",
             variable.name = "Age",
             value.name = "DiscardsNumbers")
dfpdf$PropDiscardsNumbers <-
  with(dfpdf,
       DiscardsNumbers / ave(DiscardsNumbers, Year, FUN = sum))

dmdf <- as.data.frame(dm$M)
dmdf$Year <- rownames(dmdf)
rownames(dmdf) <- NULL
dmdf <- melt(dmdf,
             id.vars = "Year",
             variable.name = "Age",
             value.name = "DiscardsBiomass")
dmdf$PropDiscardsBiomass <-
  with(dmdf,
       DiscardsBiomass / ave(DiscardsBiomass, Year, FUN = sum))


alwdf <- Reduce(
  function(x,y) merge(x, y, by = c("Year", "Age"), all = TRUE),
  list(cwdf, lwdf, dwdf, w_thrdf, dfpdf, dmdf)
)
alwdf$RevAge <- factor(alwdf$Age, levels = rev(levels(alwdf$Age)))
```
#### Catch weight at age (CWAA)
This section currently takes in pre-calculated catch-weight at age data (before splitting surviving discards out). These calculations should be integrated at the begining of the script.

```{r eval=TRUE, fig.cap= paste0("Catch weight at age by year, over time")}
...
#### Contributions of Ages to discards over time
```{r fig.cap="Number at age in discards over time."}
ggplotly(
  ggplot(data = alwdf) +
...
```{r fig.cap="Proportion of Numbers at age in discards over time."}
ggplotly(
  ggplot(data = alwdf) +
    geom_area(mapping = aes(x = Year,
                            y = PropDiscardsNumbers,
                            group = RevAge,
                            fill = RevAge)) +
    theme_few() +
    scale_fill_viridis_d(name   = "Age",
...
```{r fig.cap="Biomass at age in discards over time."}
ggplotly(
  ggplot(data = alwdf) +
    geom_area(mapping = aes(x = Year,
                            y = DiscardsBiomass,
...
```{r fig.cap="Proportion of Biomass at age in discards over time."}
ggplotly(
  ggplot(data = alwdf) +
    geom_area(mapping = aes(x = Year,
                            y = PropDiscardsBiomass,
                            group = RevAge,
                            fill = RevAge)) +
    theme_few() +
    scale_fill_viridis_d(name   = "Age",
...
#### Threshold weights at age
```{r fig.cap="Threshold weights at age (kg) over time (purple line), bounded by the landings weight at age (upper green ribbon) and the discard weight at age (lower green ribbon)"}
ggplotly(
  ggplot(data = alwdf) +
    geom_line(mapping = aes(x = Year,
                            y = ThresholdWeights,
    ...
    geom_ribbon(mapping = aes(x = Year,
                              ymin = DiscardsWeights,
                              ymax = LandingsWeights,
                              group = Age),
                colour = ebpal[2],
                fill = ebpal[2],
                alpha = 0.25) +
    ylab("Weights (kg) at age") +
...
```{r tab.cap="Annual weight-at-age threshold values for desirable fish"}
kable(w_thr, digits = 2, row.names = TRUE)
```
...
IC_agg_temp <- setNames(
  data.frame(
    Subdivision = c("SD 21", "SDs 22-32"),
    c(
      sum(IC_agg_temp[IC_agg_temp$Area == "27.3.a.21", "CATON"]),
      sum(IC_agg_temp[IC_agg_temp$Area != "27.3.a.21", "CATON"])
    ),
    c(
      sum(IC_agg_temp[IC_agg_temp$Area == "27.3.a.21" &
                      IC_agg_temp$CatchCategory == "Landings", "CATON"]),
      sum(IC_agg_temp[IC_agg_temp$Area != "27.3.a.21" &
                      IC_agg_temp$CatchCategory == "Landings", "CATON"])
    ),
    c(
      sum(IC_agg_temp[IC_agg_temp$Area == "27.3.a.21" &
                      IC_agg_temp$CatchCategory %in% c("BMS", "Discards"), "CATON"]),
      sum(IC_agg_temp[IC_agg_temp$Area != "27.3.a.21" &
                      !IC_agg_temp$CatchCategory %in% c("BMS", "Discards"), "CATON"])
    )
  ),
  c(
    "Subdivision",
    paste0("Catch ", DataYear),
    paste0("Landings ", DataYear),
    paste0("Discards ", DataYear)
  )
)
...
# Surviving discard fractions from discards and total catch - from DataCoordination Lowestoft file
...
#===
# Aggregate and format Landings and Discards for Named Countries & "Others"
#===
...
# #===
# IC_agg_temp2 <- ic_clean[ic_clean$CatchCategory %in% c("BMS landing", "BMS"), ]
# IC_agg_temp2[IC_agg_temp2$CatchCategory == "BMS landing", "CATON"] <- IC_agg_temp2[IC_agg_temp2$CatchCategory == "BMS landing", "OfficialLandings.kg"]/1000
# IC_agg_temp2 <- aggregate(CATON~MgmtArea+Year+,
#                          data = IC_agg_temp2,
#                          FUN = "sum")
# IC_agg_temp2$CountryCatch <- rep("BMS", nrow(IC_agg_temp2))
# 
# #===
# # Combine Country/catch categories and reshape to wide
# #===
# IC_agg_temp <- rbind(IC_agg_temp, IC_agg_temp2)
...
#===
# subtract BMS from Discards & Restructure
#====
...
```{r CalcsForSurveyBiologicalData, results=FALSE, warning=FALSE, message=FALSE, error=FALSE}
...
# #===
# source(file = "SupportingScripts/01_DatrasDownloads.R")
# #====
...
```{r MOTable_AnnualEstimates, tab.cap=paste0("Maturity Ogives of Ple.27.21-32, using annual estimates from surveys for the whole timeseries, 2002:", DataYear+1)}
...
kable(moRawOrder, digits = 2)
```


```{r MOTable_sw5, tab.cap=paste0("Maturity Ogives of Ple.27.21-32, using a 5 year sliding window mean approach.  Values are timeseries means of the whole timeseries, 2002:", DataYear+1)}
...
```{r tab.cap=="Five-year sliding window mean of maturity at age, with age 1 set to 0, post-hoc."}
mo5swL_1zero <- mo5swL
...
### Condition Factor
As developments in stock-weights at age, maturity at age, and natural mortality have been observed, we also want to check how condition factors have been varying.  Furthermore, condition factor is often corellated with length, so with increasing proportions of small fish, overall stock condition factor may start to look like it is decreasing.  Therefore this effect needs to be either modelled, or condition only compared within discretised length groups. (WORK IN PROGRESS)

```{r conditionFactor, eval=FALSE}
...
```{r fig.cap="Time and age varying natural mortality based on Gislason. Ages overlayed for Jesper.", fig.height=8}
plt_nmvar_age <- ggplot(nm_var) +
  geom_line(mapping = aes(x = year,
                          y = nm,
                          group = Age,
...
```{r fig.cap= "Bubble plot showing RELATIVE (within year) numbers (size of bubble) at age (y-axis), over time (x-axis) in catches.  The dotted lines show the progression of co-horts where large and small numbers at age can be tracked year to year.  For 'Rie!"}
#===
# Data Prep
#====
relCatchNum <- catchNum
relCatchNum[, -1] <- catchNum[, -1]/rowSums(catchNum[, -1])

relCatchNumLng <- reshape(data = relCatchNum,
                       varying = list(colnames(relCatchNum)[2:length((colnames(relCatchNum)))]),
                       v.names = c("Number"),
                       timevar = "Age",
                       direction = "long")
relCatchNumLng$Year <- as.numeric(as.character(relCatchNumLng$Year))
#=====

#===
# Figure Cohorts over time from catches
#====
plt_relCatchCohorts <- ggplot() +
  geom_line(data = catchcohortsLng,
            mapping = aes(x = Year,
                          y = Age,
                          group = Cohort),
            linetype = "dotted") +
  geom_point(data = relCatchNumLng,
             mapping = aes(x = Year,
                           y = Age,
                           size = Number)) +
  theme_clean() +
...
```{r fig.cap= paste0("Bubble plot showing numbers (size of bubble) at age (y-axis), over time (x-axis) estimated from survey indices. The dotted lines show the progression of co-horts where large and small numbers at age can be tracked year to year.")}
#===
# Data Prep
#====
...
```{r fig.cap= "Bubble plot showing RELATIVE (within year) numbers (size of bubble) at age (y-axis), over time (x-axis) estimated from surveys.  The dotted lines show the progression of co-horts where large and small numbers at age can be tracked year to year."}
#===
# Data Prep
#====
relSurvTun <- survTun
relSurvTun[, 3:ncol(relSurvTun)] <- survTun[, 3:ncol(survTun)]/rowSums(survTun[, 3:ncol(survTun)])

relSurvTunLng <- reshape(data = relSurvTun,
                       varying = list(colnames(relSurvTun)[3:length((colnames(relSurvTun)))]),
                       v.names = c("RelativeIndex"),
                       timevar = "Age",
                       direction = "long",
                       idvar = c("Year", "Survey"))
relSurvTunLng$Year <- as.numeric(as.character(relSurvTunLng$Year))
#=====

#===
# Plot
#=====
plt_relSurvCohorts <- ggplotly(
  ggplot() +
  geom_point(data = relSurvTunLng,
             mapping = aes(x = Year,
                           y = Age,
                           size = RelativeIndex),
             alpha = 0.5) +
  ...
  coord_cartesian(xlim = c(1999,DataYear)) +
  facet_grid(rows = vars(Survey)) +
  scale_y_continuous(limits = c(1, 7), breaks = seq(1, 7, by = 3)) +
  # scale_size(range = c(1, 6)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
)


for (i in seq_along(plt_relSurvCohorts$x$layout)) {
  if (grepl("^yaxis", names(plt_relSurvCohorts$x$layout)[i])) {
    plt_relSurvCohorts$x$layout[[i]]$autorange <- FALSE
  }
}

plt_relSurvCohorts

k <- append(k, "relSurvTunLng")
rm(list = ls()[!ls() %in% k])
#======
```